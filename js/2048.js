/**
 * Created by allenzc on 2016/12/7.
 */
var game={
    RN:4,CN:4,
    data:null,
...
document.getElementById('save').onclick=function(){
    this.nextSibling('div').style.display='block';
}
document.getElementById('submit').onclick=function(){
    var player = document.getElementsByClassName('player')[0].value;
    ...
    xhr.open('POST','data/score_2048.php',true);
    xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
    xhr.send(`player=${player}&code=${code}&score=${score}`);
}
...
function show(){
    ...
    xhr.open("GET","data/show_2048.php",true);
    xhr.send('null');
}

document.getElementById('save').onclick=function(){

    this.parentNode.parentNode.className = 'save';
...
document.getElementById('submit').onclick=function(){
    ...
    function deal(xhr){
        if(1){
            _this.parentNode.parentNode.className='submit';

        }
    }
    xhr.open("GET",'data/show_2048.php?player='+player+'&code='+code+'&score='+score,true);
    xhr.send(null);
}